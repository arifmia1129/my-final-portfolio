    ...
    return (
        <div className='my-10'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-10 my-5'>
                ...
                {
                    img?.map((image, index) => <div key={index} className='h-44 overflow-y-scroll'>
            ...
            <div>
                ...
                {
                    link?.map((l, i) => <a key={i} target="_blank" rel="noreferrer" href={l?.link} className="btn bg-gradient-to-r from-secondary to-primary text-white font-bold uppercase border-0 m-1">{l?.name}
                    </a>)
                }
            </div>
        </div>
